...
class AudioIn:
    def __init__(self, wav_path, freq_range=[00, 3100]):
        ...
        self.zgrid_main = np.ones((HOPS_PER_CYCLE * 2, self.nFreqs), dtype = np.complex64) 
        samples_perhop = int(SAMP_RATE / (SYM_RATE * HPS))
        self.zgrid_main_ptr = 0
    ...
    def process_hop(self, in_data):
        ...
        self.zgrid_main[self.zgrid_main_ptr, :] = np.fft.rfft(self.fft_in)[:self.nFreqs]
        self.zgrid_main_ptr +=1
...
def get_messages():
    ...
    for phi_sym in np.linspace(-np.pi, np.pi, 10):
        print(phi_sym)
        for fb in range(nFreqs - 8 * BPT):
            ...
            max0 = np.argmax(cand_z[0,:])
            cand_z /= cand_z[0, max0]
            correl = np.exp([[1j * phi_sym * s for t in range(8)] for s in hops])
            dBgrid = 20*np.log10(np.abs((cand_z * correl).real) + 1e-12)
            pmax = np.max(dBgrid)
