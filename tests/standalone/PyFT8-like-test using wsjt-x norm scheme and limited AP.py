...
def get_messages(wav_file):
    global audio_buffer, fft_in, zgrid_main, zgrid_main_ptr
    ...
    for fb in range(nFreqs - 8 * BPT):
        ...
        dBstrip = 20*np.log10(np.abs(zstrip))

        sync = {'h0_idx':0, 'score':0}
        for h0_idx in range(H0_RANGE[0], H0_RANGE[1]):
            sync_score = float(np.dot(dBstrip[h0_idx + BASE_COSTAS_HOPS + 36 * HPS, :7*BPT].ravel(), csync_flat))
            test_sync = {'h0_idx':h0_idx, 'score':sync_score}
            if test_sync['score'] > sync['score']:
                sync = test_sync
                
        hops = [h % hops_per_grid for h in sync['h0_idx'] + BASE_PAYLOAD_HOPS]
        p = np.abs(zstrip[hops, :])
        ...
        llr_sqrd_mean = np.mean(llr**2)
        llr_var = llr_sqrd_mean - llr_mean
        llr0 = 2.83 * llr / np.sqrt(llr_var) if llr_var > 0 else 2.83 * llr / np.sqrt(llr_sqrd_mean)
            ...
            if(bits77_int):
                ...
                if msg:
                    ...
                    if(msg not in messages):
                        h0_idx = sync['h0_idx']
                        messages[msg] = f"{1+len(messages):03d}:{msg:25s}{6.25*fb/BPT:7.1f}, {0.16*h0_idx/HPS - 0.5:5.1f} {nits:03d} {ipass:03d}"
                        print(messages[msg])
    return messages
