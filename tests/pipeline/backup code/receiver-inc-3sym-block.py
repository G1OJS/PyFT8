...
class Candidate:
    ...
    def _get_ch_llrs_from_grid(self):
        ...
        self.snr = int(np.max(p) - np.min(p) - 58)
        if self.snr >= -24:
            self.snr = np.clip(self.snr, -24, 24)
    ...
    def _get_ch_llrs_from_spectrum(self):
        ...
        if self._count_costas_maxima() <= 6:
            self.decode_result = 'stop'
            return
        ...
        self.snr = int(np.max(dB) - np.min(dB) - 58)
    ...
    def manage_cycle(self):
        ...
        while True:
            ...
            if len(to_decode):
                ...
                to_decode.sort(key=lambda c: float(c.snr), reverse=True)
