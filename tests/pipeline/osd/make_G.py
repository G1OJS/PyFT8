...
from PyFT8.transmitter import ldpc_encode
G = np.zeros((91,174), dtype=np.uint8)
m91_int = 1 << 90
for i in range(91):
    bits91_int = m91_int 
    if i > 14:
        crc14_int = 0
        for j in range(96):
            inbit = ((bits91_int >> (76 - i)) & 1) if i < 77 else 0
            bit14 = (crc14_int >> (14 - 1)) & 1
            crc14_int = ((crc14_int << 1) & ((1 << 14) - 1)) | inbit
            if bit14:
                crc14_int ^= 0x2757
        bits91_int = bits91_int | crc14_int
 
    bits174_int, bits83_int = ldpc_encode(bits91_int)
    G[i,:] = [int(b) for b in f"{bits174_int:0174b}"]
    m91_int = m91_int >> 1
