...
def gf2_systematic_from_reliability(reliab_order):
    global G
    G = (G.copy() & 1).astype(np.uint8)
    k, n = G.shape
    # Reorder, most reliable into identity submatrix
...
def osd_decode_minimal(llr, order=1, L=20):
    global G
    ...
    Gsys, colperm = gf2_systematic_from_reliability(reliab_order)
    r_sys = r[colperm]
    w_sys = w[colperm]
    u0 = r_sys[:k].copy()
