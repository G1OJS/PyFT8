...
def spectrum(audio, max_freq = 300):
    ...
    spec = np.ones((HOPS_PER_CYCLE, nFreqs), dtype = np.float32)
...
symbols = (8*np.random.rand(79)).astype(int)
audio = create_ft8_wave(symbols, added_noise = 0)
print(audio.shape)
spec, df = spectrum(audio)
