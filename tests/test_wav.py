...
def non_threaded_decode(audio_in):
    ...
    demod.audio_loaded_at = timers.tnow()
...
def threaded_decode():
    ...
    while not cycle_manager.spectrum.searched:
        sleep(0.1)
    while True:
...
def onDecode(c):
    decoded_candidates.append(c)
    dd = c.decode_dict
    t_after_load = timers.tnow()-cycle_manager.audio_loaded_at
    print(f"{t_after_load:7.2f} {dd['call_a']:>8} {dd['call_b']:>8} {dd['grid_rpt']:>8} {c.frozen_at_hop} {c.score:8.3f} {c.llr_sd:8.3f} {c.synced_pwr/1e9:8.3f} {c.snr:8.0f} "
      +f"{c.origin[0]:>8} {c.origin[1]:>8} {c.n_its:>8}")
...
heads = ['Tload+', 'Rx call', 'Tx call', 'GrRp', 'Frozen at hop', 'SyncScr', 'LLR_sd', 'snr', 't0', 'f0', 'iters']
