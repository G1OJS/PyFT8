...
class receiver:
    def __init__(self, audio_in, freq_range, on_decode, waterfall):
        ...
        self.nFreqs = audio_in.nFreqs
        self.dt = 1.0 / (SYM_RATE * HPS) 
        self.df = freq_range[1] / (self.nFreqs -1)
        self.n_origins = self.nFreqs - 8 * BPT
    ...
    def run_receiver(self):
        def check_ptr_alarm(ptr_alarm):
            h = (self.audio_in.dBgrid_main_ptr - ptr_alarm[0]) % self.audio_in.hops_per_cycle
            ring =  h < ptr_alarm[1]
            ptr_alarm[1] = h
            return ring
            ...
            if(ptr != main_ptr_prev):
                ...
                for idx, origin in enumerate(origins_for_decode):
                    if (ptr - origin['h0']) > LAST_BASE_PAYLOAD_HOP and not origin['demap_started']:
                        ...
                        origin = demap(self.audio_in.dBgrid_main, origin)
                    if float(origin['llr_sd']) > 0 and origin['td'] == 0:
                        to_decode_this_loop.append(origin)
                    if origin['msg'] and origin['msg'] not in self.duplicate_filter:
                        self.duplicate_filter.add(origin['msg'])
                        if self.waterfall:
                            self.waterfall.post_decode(origin['h0'], origin['f0'], origin['msg'])
                        self.on_decode(origin)

                to_decode_this_loop.sort(key=lambda o: o['llr_sd'], reverse=True)
                for origin in to_decode_this_loop[:35]:
                    origin = decode(origin, self.ldpc)
        
                if check_ptr_alarm(search_alarm):
                    tprint("Start search")
                    cycle = int(self.audio_in.dBgrid_main_ptr / self.audio_in.hops_per_cycle)
                    cycle_h0 = cycle * self.audio_in.hops_per_cycle
                    cs = cyclestart_str(time.time())
                    origins_for_decode = []
                    for f0 in range(self.n_origins):
                        origin = {'h0':0, 'f0': f0, 'llr_sd':0, 'llr':[], 'td':0, 'n_its':0, 'demap_started':0,
                        'decoder': 'PyFT8', 'cs':cs, 'sync_idx': 1, 'sync_score': -1e20,
                        'dt':0, 'f':int(f0*self.df), 'msg_tuple':(''), 'msg':'', 'ncheck0': 99, 'snr': -30}
                        freq_idxs = f0 + BASE_FREQ_IDXS
                        p_dB = self.audio_in.dBgrid_main[:, f0:f0 + 8 * BPT]
                        for h0 in range(cycle_h0 + H0_RANGE[0], cycle_h0 + H0_RANGE[1]):
                            sync_score = float(np.dot(p_dB[h0 + BASE_COSTAS_HOPS + 36 * HPS, :].ravel(), self.csync_flat))
                            if sync_score > origin['sync_score']:
                                origin.update({'h0':h0, 'sync_score':sync_score, 'dt': h0*self.dt - 0.7})
                        origins_for_decode.append(origin)
                    tprint("Search done")
