...
class Waterfall:
    def __init__(self, specbuff, hops_persymb, fbins_pertone, costas, t0=0, t1=15, f0=100, f1=3500):
        ...
        self.extent = [self.specbuff.freqs[0], self.specbuff.freqs[-1]+df,
                       self.specbuff.times[0],  self.specbuff.times[-1]+dt]
    ...
    def update(self, title = "FT8 Waterfall", candidates = None, show_n_candidates = 0):
        ...
        if(candidates):
            df = np.diff(self.specbuff.freqs).mean()
            dt = np.diff(self.specbuff.times).mean()
            for i, c in enumerate(candidates):
                t0_c = self.specbuff.times[c.tbin_idx]
                f0_c = self.specbuff.freqs[c.fbin_idx]
                dt_c = self.specbuff.times[c.tbin_idx + self.hops_persymb * c.num_symbols - 1] - self.specbuff.times[c.tbin_idx] + dt
                df_c = self.specbuff.freqs[c.fbin_idx + self.fbins_pertone * c.tones_persymb - 1] - self.specbuff.freqs[c.fbin_idx] + df
    ...
    def show_zoom(self, c, t0, t1, f0, f1):
        ...
        for symb_idx, tone_idx in enumerate(self.costas):
            for symb_copy_idx in [symb_idx, symb_idx+36, symb_idx+72]:
                ...
                dt_sync = self.hops_persymb * dt
                df_sync = self.fbins_pertone * df
