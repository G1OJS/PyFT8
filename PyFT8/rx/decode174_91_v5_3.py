...
class LDPC174_91:
    ...
    def decode(self, llr):
        ...
        while True:
            ...
            check_ids = np.arange(self.kM)[None, None, :]
            exclude = (self.kMN[:, self.VN] - 1) == check_ids  
            excl_vals = (tov_gather * exclude).sum(axis=0)
            ...
            for kk in range(self.kNCW):
                for variable_node in range(self.kN):
                    ...
                    tvals = tanhtoc[:self.neigh_counts_for_ichk[ichk], ichk][mask]
                    Tmn = np.prod(tvals)
