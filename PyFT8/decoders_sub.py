...
def crc_unpack91(codeword91):
    ...
    if(bits77_int > 0):
        ...
        if crc14_int == bits91_int & 0b11111111111111:
            ...
            return msg_tuple, symbols 
    return None, None

#============== GOOD91 ===========================================================
def good91(llr):
    msg_tuple, symbols = crc_unpack91(llr[:91])
    return msg_tuple, symbols
...
def ldpc_decode(llr, max_ncheck0, max_iters):
    ...
    for n_its in range(max_iters):
        ...
        if ncheck <= max_ncheck0 or n_its == 0:
            if ncheck == 0:
                msg_tuple, symbols = crc_unpack91(llr[:91])
                if msg_tuple:
                    return msg_tuple, symbols, llr
            else:
...
def osd_01(llr):
    ...
    for i, bit in enumerate(fliplist):
        ...
        if score < best:
            best = score
            msg_tuple, symbols = crc_unpack91(cw)
            if msg_tuple:
                return msg_tuple, symbols
    return None, None
